...
app.use((req, res, next) => {
  ...
  console.log(`🔑 Auth: ${req.headers.authorization ? 'Bearer ' + req.headers.authorization.substring(7, 30) + '...' : 'None'}`);
  ...
  res.json = function(data) {
    console.log('\n' + '-'.repeat(80));
    console.log(`📤 OUTGOING RESPONSE: ${req.method} ${req.path}`);
...
const MCP_VERSION = '2024-11-05'; // We support this version
const REQUIRE_AUTH = process.env.REQUIRE_AUTH !== 'false'; // Default to true, set to false to disable

console.log(`🔐 Authentication: ${REQUIRE_AUTH ? 'ENABLED' : 'DISABLED (TESTING MODE)'}`);

// If no auth, we should not advertise OAuth endpoints
if (!REQUIRE_AUTH) {
  console.log('⚠️  OAuth metadata endpoints disabled');
}
// In-memory storage
...
app.get('/.well-known/oauth-authorization-server', (req, res) => {
  ...
  // If auth is disabled, don't advertise OAuth
  if (!REQUIRE_AUTH) {
    console.log('⚠️  Auth is disabled - returning 404');
...
app.get('/.well-known/oauth-protected-resource', (req, res) => {
  // If auth is disabled, don't advertise OAuth
  if (!REQUIRE_AUTH) {
    console.log('⚠️  OAuth metadata requested but auth is disabled - returning 404');
    return res.status(404).json({
      error: 'not_found',
      message: 'OAuth is disabled on this server'
    });
...
// Handle SSE connection attempts (Claude might try this first)
app.get('/mcp', (req, res) => {
  console.log('\n🔍 SSE Connection attempt detected (GET /mcp)');
  console.log('   Claude.ai is trying Server-Sent Events transport');
  console.log('   ⚠️  SSE not implemented - client should fallback to HTTP POST');
  
  // Return 405 Method Not Allowed to force Claude to use POST
  res.status(405).json({
...
app.post('/mcp', async (req, res) => {
  ...
  // Check authentication (skip if REQUIRE_AUTH is false)
  if (REQUIRE_AUTH) {
    ...
    // No auth mode - use dummy username
  ...
  if (msg.method === 'initialize') {
    console.log(`✅ MCP initialized: ${username}`);
    
    // Support both old and new protocol versions
    const clientVersion = msg.params?.protocolVersion || MCP_VERSION;
    console.log(`   Client protocol version: ${clientVersion}`);
    console.log(`   Server protocol version: ${MCP_VERSION}`);
    
    // Use the client's protocol version if it's newer
    const agreedVersion = clientVersion > MCP_VERSION ? clientVersion : MCP_VERSION;
    console.log(`   Using protocol version: ${agreedVersion}`);
    
    return res.json({
      jsonrpc: '2.0',
      result: {
        protocolVersion: agreedVersion, // Match client version
        capabilities: { 
          tools: { listChanged: true }, // Indicate we support tool list changes
          resources: {},
          prompts: {}
        },
  ...
  if (msg.method === 'notifications/initialized') {
    console.log(`✅ Client acknowledged initialization`);
    // Notifications in JSON-RPC don't have an id and don't require a response
    // Just return 200 OK with no body
    if (!msg.id) {
      console.log(`   This is a notification (no id) - not sending response`);
      return res.status(200).end();
    }
    // If it has an id (older clients), send empty result
    return res.json({
      jsonrpc: '2.0',
      result: {},
      id: msg.id
  ...
  if (msg.method === 'tools/list') {
    console.log(`📋 Listing tools for: ${username}`);
    const toolsResponse = {
      jsonrpc: '2.0',
      result: {
        tools: [
    ...
    console.log('📤 Returning tools:', JSON.stringify(toolsResponse, null, 2));
  ...
  if (msg.method === 'tools/call') {
    ...
    console.log(`🛠️  Tool called: ${name} by ${username}`);
      ...
      return res.json({
        ...
        result: {
          content: [{
            ...
            text: JSON.stringify({
              username: username,
              mode: REQUIRE_AUTH ? 'authenticated' : 'anonymous',
    ...
    if (name === 'echo') {
      return res.json({
        jsonrpc: '2.0',
        result: {
          content: [{ type: 'text', text: `Echo from ${username}: ${args.message}` }]
        },
        id: msg.id
      });
    }
