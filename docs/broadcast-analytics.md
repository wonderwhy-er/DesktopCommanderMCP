...
The device records one `received` event with `params.transport=broadcast` when a targeted `new_call` callback enters. Other checkpoints likewise use their stage as the event name. It captures UTC and `performance.now()` before the existing doorbell handler fetches or claims the call. The event carries `call_id`, `device_id`, `notification_id`, `attempt_number`, `observation_id`, and `source_process_id` for correlation with Remote MCP's dispatch observations. Duplicate callbacks produce separate receipts. Other devices on the per-user channel and malformed identifiers produce none.
