...
# Installed before anything is created, not after the report: these units carry
# Restart=always with no start-rate limit, so an interrupted sleep would leave
# them restarting for as long as the session lives, and an interrupt before
# that would strand the unit files and work directories.
cleanup() {
