...
assert.deepEqual(config.args, [path.join(repoRoot, 'dist', 'index.js')]);assert.equal(
  config.cwd,
  os.homedir(),
  'local MCP child should use the durable user home even if launcher cwd disappears'
);
assert.notEqual(
  config.cwd,
